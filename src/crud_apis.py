...
def create_subscription(
    ...
    # Check if the subscription already exists for the user and magazine
    existing_subscription = (
        db.query(Subscriptions)
        .filter(
            Subscriptions.user_id == subscription.user_id,
            Subscriptions.magazine_id == subscription.magazine_id,
            Subscriptions.plan_id == subscription.plan_id,
        )
        .first()
    )

    magazine = (
        db.query(Magazines).filter(Magazines.id == subscription.magazine_id).first()
    )
    ...
    if subscription.price <= 0:
        ...
        if subscription.price <= 0:
            ...
            raise HTTPException(
                status_code=422,
                detail=f"{subscription.price} Subscription already exists",
            )
